...
start_ngrok() {
    ...
    ngrok ${NGROK_PROTOCOL} ${NGROK_PORT} >/dev/null 2>&1 &
    disown
    sleep 3
...
# Trap Ctrl+C
trap 'echo ""; log "Stopping monitor..."; pkill -9 ngrok 2>/dev/null; exit 0' INT TERM

# Check if Telegram credentials are set
if [ "$TELEGRAM_BOT_TOKEN" == "YOUR_BOT_TOKEN_HERE" ] || [ "$TELEGRAM_CHAT_ID" == "YOUR_CHAT_ID_HERE" ]; then
    log "Error: Please set your Telegram bot token and chat ID in the script"
    exit 1
fi
